"""
MCQ সলভার v2.1 - মাল্টি API Key অটো রোটেট
"""
...
def ask_gemini(prompt):
    ...
    for _ in range(len(GEMINI_KEYS)):
        ...
        try:
            ...
            print(f"  Gemini key {gemini_index} skip")
            time.sleep(0.3)
    return None
...
def ask_groq(prompt):
    ...
    for _ in range(len(GROQ_KEYS)):
        ...
        try:
            ...
            print(f"  Groq key {groq_index} skip")
            time.sleep(0.3)
    return None


def parse_mcqs(raw_text):
    prompt = f"""নিচের টেক্সট থেকে শুধু MCQ extract করো।
নিয়ম:
- শুধু ৪ অপশন + ১ উত্তর আছে এমন প্রশ্ন নাও
- ওয়াটারমার্ক, সাইট নাম, প্রমো, "কোন সালে এসেছিল" বাদ দাও
JSON array দাও (শুধু JSON):
[{{"question_no":1,"question":"প্রশ্ন","option_a":"ক","option_b":"খ","option_c":"গ","option_d":"ঘ","correct_answer":"ক","correct_option_text":"টেক্সট"}}]
...
def parse_image(image_path):
    ...
    try:
        ...
        prompt = """ছবি থেকে MCQ extract করো। ওয়াটারমার্ক বাদ দাও।
JSON: [{"question_no":1,"question":"...","option_a":"...","option_b":"...","option_c":"...","option_d":"...","correct_answer":"ক","correct_option_text":"..."}]"""
...
def export_pdf():
  ...
  def build_html(mcqs, header, for_print=False):
    ...
    if header and any(v for v in header.values() if v):
        hh = f"""<div class="eh"><h1>{header.get('institution','')or'প্রতিষ্ঠান'}</h1><h2>{header.get('exam_name','')or'পরীক্ষা'}</h2><table class="it"><tr><td><b>সাল:</b> {header.get('year','-')}</td><td><b>পূর্ণমান:</b> {header.get('total_marks','-')}</td><td><b>সময়:</b> {header.get('duration','-')}</td></tr><tr><td><b>নেগেটিভ:</b> {header.get('negative_marking','-')}</td><td colspan="2"><b>নির্দেশনা:</b> {header.get('instructions','-')}</td></tr></table></div>"""
    qs = ""
...
def build_word(mcqs, header):
    ...
    for i, m in enumerate(mcqs, 1):
        ...
        for lbl, key in [('ক','option_a'),('খ','option_b'),('গ','option_c'),('ঘ','option_d')]:
            doc.add_paragraph(f"    ({lbl}) {m.get(key, '')}").paragraph_format.space_after = Pt(2)
        ...
        if m.get('sources'):
            ...
            rs = ps.add_run(f"📚 তথ্যসূত্র: [{', '.join(m['sources'])}]")
            rs.font.size = Pt(9.5)
...
if __name__ == '__main__':
    ...
    print(f"MCQ Solver v2.1 | AI:{ai_count} Keys:{len(GEMINI_KEYS)+len(GROQ_KEYS)} Port:{port}")
